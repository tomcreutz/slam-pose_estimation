...
public:
  ...
  // todo(tcreutz): Should time be handled in nanoseconds? As rclcpp Time / timestamps are usually held in nanoseconds
  ...
  UnscentedKalmanFilter(const UnscentedKalmanFilter &) = delete;
  UnscentedKalmanFilter & operator=(const UnscentedKalmanFilter &) = delete;
  ...
  const std::chrono::microseconds & getLastMeasurementTime() const
  {
    return last_measurement_time_usec;
  }
...
protected:
  ...
  void checkMeasurment(
    const Eigen::Matrix<scalar_type, DIM, 1> & mu,
    const Eigen::Matrix<scalar_type, DIM, DIM> & cov) const
  {
